...
const ListCard = ({list,serviceId}: Props) => {
  ...
  return (
    ...
    <p>Begin datum: {formatDate(list.startDate)}</p>
    <p>Einddatum: {formatDate(list.endDate)}</p>
    <div className="mt-4 flex items-center gap-3">
      <Button asChild><Link href={`/dashboard/services/${serviceId}/pricing/${list.id}`} className="flex items-center gap-3 "><Edit size={20}/>
Bewerk</Link></Button>
      <Button onClick={()=>setOpen('delete-modal',{deleteFn:()=>deleteList(serviceId,list.id)})} variant={'destructive'}><Delete className='mr-3' /> Verwijderen</Button>
      </div>
  </div>
  )
}
