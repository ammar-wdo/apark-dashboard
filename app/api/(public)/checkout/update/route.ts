...
export async function POST(req: Request) {
  ...
  try {
    ...
    if(!bookingCode) return new NextResponse('Invalid credintials, bookingCode is required',{status:400})
   ...
   const validBody = bookingSchema.safeParse(rest);
    if (!validBody.success)
      ...
      if(!booking) return new NextResponse('Invalid credintials',{status:400})
    ...
    if (!available)
     ...
     if(newPakingDays > booking?.daysofparking){
      ...
      const validService = isServiceValid(service,newArrival.toString(),newDeparture.toString(),booking.id,newDays)
      if(!validService) return new NextResponse('This service is no more available',{status:400})

      const newPrice = findTotalPrice(service,newDays+booking?.daysofparking,newArrival.toString(),newDeparture?.toString()) 
...
await prisma.notification.create({
    data:{
        ...
        const session = await stripe.checkout.sessions.create({
            payment_intent_data: { metadata: { id: booking.id,arrivalDate:booking.arrivalDate.toString(),departureDate:booking.departureDate.toString(),total:newPrice,daysofparking:newPakingDays},
            ...
            mode: "payment",
            metadata: { id: booking.id,arrivalDate:newArrival.toString(),departureDate:newDeparture.toString(),total:newPrice,daysofparking:newPakingDays},
...
// const {serviceId,...rest} = validBody.data
